...
namespace MatthewPageUK\LaraVelDevBuddy\Support\Uml;
...
class LaravelToUml {
    ...
    protected function getStyling() {
        $styles = [];
        foreach(config('lara-vel-dev-buddy.laravel-to-uml.style') as $key => $value) {
            $styles[] = "#{$key}: $value";
        }
        return $styles;
    }
    ...
    protected function getListOfFiles() {
        ...
        $excludes = [];
        foreach(config('lara-vel-dev-buddy.laravel-to-uml.directories') as $configKey => $folder) {
            if(!config("lara-vel-dev-buddy.laravel-to-uml.{$configKey}")) $excludes[] = $folder;
        }
        ...
        $finder
            ...
            ->name("*.php")
            ->notPath([...config('lara-vel-dev-buddy.laravel-to-uml.excludeFiles'), ...$excludes])
            ->sortByName();
        return $finder;
    }
